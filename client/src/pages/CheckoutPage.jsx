    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-40 px-4">
      <div className="bg-white rounded-md shadow-2xl p-6 max-w-sm w-full text-center">
        ...
        <div className="flex gap-3">
          <button
            onClick={onClose}
            className="flex-1 border border-gray-300 text-gray-600 py-2 rounded-sm text-sm font-semibold hover:bg-gray-50"
          >
            Cancel
          </button>
          <button
            onClick={onConfirm}
            className="flex-1 bg-[#2874f0] text-white py-2 rounded-sm text-sm font-bold hover:bg-blue-700"
          >
            Confirm Order
          </button>
        </div>
...
export default function CheckoutPage() {
  ...
  const handlePayClick = () => {
    ...
    if (payment === 'CARD') {
      ...
      if (card.expiry.replace(/\s/g, '').length < 5) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const orderedItemsSnapshot = items.map((item) => ({
      ...
      if (orderedItemsSnapshot.length > 0) {
        dispatch(updateProductStockLocally(orderedItemsSnapshot));
      }
                      ...
                      <div>
                        ...
                        <div className="relative">
                          ...
                          <input
                            ...
                            className={`w-full border rounded-sm pl-8 pr-3 py-2.5 text-sm focus:outline-none focus:ring-1 transition-colors ${
                              upiError ? 'border-red-400' : 'border-gray-300 focus:border-[#2874f0] focus:ring-[#2874f0]'
                            }`}
                          />
                        </div>
                        ...
                        <div className="flex gap-2 mt-3 flex-wrap">
                          {['GPay', 'PhonePe', 'Paytm', 'BHIM'].map((app) => (
                            <button
                              key={app}
                              onClick={() => {
                                setUpiId(`demo@${app.toLowerCase()}`);
                                setUpiError('');
                              }}
                              className="text-xs border border-gray-200 px-2.5 py-1 rounded hover:border-[#2874f0] hover:text-[#2874f0] transition-colors"
                            >
                              {app}
                            </button>
                          ))}
                    ...
                    {payment === 'CARD' && (
                      <div>
                        ...
                        <div
                          ...
                          style={{
                            background: cardLogo
                              ? `linear-gradient(135deg, ${cardLogo.color}cc, ${cardLogo.color})`
                              : 'linear-gradient(135deg, #2874f0cc, #2874f0)',
                          }}
                        >
                          ...
                          <p className="tracking-widest text-sm font-mono mb-3 opacity-90">
                            {card.number || '•••• •••• •••• ••••'}
                          </p>
                        ...
                        <div className="space-y-3">
                          <div>
                            ...
                            <input
                              ...
                              maxLength={19}
                              className={`w-full border rounded-sm px-3 py-2.5 text-sm font-mono focus:outline-none focus:ring-1 transition-colors ${
                                cardError ? 'border-red-400' : 'border-gray-300 focus:border-[#2874f0] focus:ring-[#2874f0]'
                              }`}
                            />
                          </div>
                          ...
                          <div className="grid grid-cols-2 gap-3">
                            <div>
                              ...
                              <input
                                ...
                                placeholder="MM / YY"
                                maxLength={7}
                                className={`w-full border rounded-sm px-3 py-2.5 text-sm focus:outline-none focus:ring-1 transition-colors ${
                                  cardError ? 'border-red-400' : 'border-gray-300 focus:border-[#2874f0] focus:ring-[#2874f0]'
                                }`}
                              />
                            </div>
                            ...
                            <div>
                              ...
                              <div className="relative">
                                <input
                                  ...
                                  className={`w-full border rounded-sm px-3 py-2.5 text-sm focus:outline-none focus:ring-1 transition-colors pr-8 ${
                                    cardError ? 'border-red-400' : 'border-gray-300 focus:border-[#2874f0] focus:ring-[#2874f0]'
                                  }`}
                                />
                                <button
                        ...
                        {cardError && (
                          <p className="text-red-500 text-xs mt-2 bg-red-50 border border-red-100 px-3 py-2 rounded-sm">
                            {cardError}
                          </p>
                        )}
                      ...
                      <div>
                        ...
                        <div className="grid grid-cols-2 gap-2 mb-3">
                          {[
                            ...
                            <label
                              key={bank.id}
                              className={`flex items-center gap-2 border rounded px-3 py-2.5 cursor-pointer text-sm transition-colors ${
                                selectedBank === bank.id
                                  ? 'border-[#2874f0] bg-blue-50 text-[#2874f0] font-semibold'
                                  : 'border-gray-200 hover:border-[#2874f0] text-gray-700'
                              }`}
                            >
                              <input
                                type="radio"
